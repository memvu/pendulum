...
void updatePosition(Ball *balls, float *theta) {
   ...
   balls[3].updatePosition(balls[2].getPosition() + ROD_LENGTH * glm::vec3(sin(theta[2]), -cos(theta[2]), 0));
}
...
static inline void mass_gravity_coriolis(
    const Vec3 &th, const Vec3 &w, double m, double l, double g, Mat3 &M, Vec3 &h, Vec3 &G) {
   ...
   dJ3w << -l * s1 * w1, -l * s2 * w2, -l * s3 * w3, l * c1 * w1, l * c2 * w2, l * c3 * w3;

   M = m * (J1.transpose() * J1 + J2.transpose() * J2 + J3.transpose() * J3);

   Eigen::Vector2d z2(0.0, 0.0);
   h = m * (J1.transpose() * (dJ1w * Vec3(w1, w2, w3)) + J2.transpose() * (dJ2w * Vec3(w1, w2, w3)) +
            J3.transpose() * (dJ3w * Vec3(w1, w2, w3)));

   Eigen::RowVector3d Jy_sum = (J1.row(1) + J2.row(1) + J3.row(1));
   G = m * g * Jy_sum.transpose();
}

Vec6 dydt(const Vec6 &y) {
   const double m = 1.0;
   const double l = ROD_LENGTH;
